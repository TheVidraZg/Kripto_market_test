...
class FrmDashboard(tk.Frame):
    def __init__(self, master, background='#333333', items_number=4):
        ...
        self.frm_trend = FrmScrollable(self,
                                  background=background)
        self.frm_trend.grid(row=0, column=1,
                            padx=10, pady=10, sticky='ew')
        
        self.frm_trend.frm_scrollable.grid_columnconfigure(0, weight=1)
        self.frm_trend.frm_scrollable.grid_columnconfigure((1, 2), weight=2)

        for i in range(items_number):
            self.lbl_crypto_color_code = tk.Label(self.frm_trend.frm_scrollable,
                                                  foreground='#FFFFFF',
                                                  background=background,
                                             text=f'CC {i}')
            ...
            self.lbl_crypto_name = tk.Label(self.frm_trend.frm_scrollable,
                                            foreground='#FFFFFF',
                                            background=background,
                                             text=f'NAME {i}')
            ...
            self.lbl_crypto_trend = tk.Label(self.frm_trend.frm_scrollable,
                                             foreground='#FFFFFF',
                                             background=background,
                                             text=f'TREND {i}%')
